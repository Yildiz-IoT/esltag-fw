 ...
 */
static void draw_bar(uint8_t *bitmap, uint16_t x, uint16_t width,
 ...
 */
static uint16_t draw_pattern(uint8_t *bitmap, uint8_t pattern, uint16_t x,
 ...
 * @return Number of pixels drawn (width)
 */
static uint16_t draw_guard(uint8_t *bitmap, uint16_t x, uint8_t pattern,
                           uint8_t pattern_width, uint16_t module_width,
                           uint16_t height, uint16_t bitmap_width)
{
    uint16_t pos = x;
...
barcode_error_t barcode_render(const char *digits, barcode_type_t type,
                               uint8_t *bitmap, uint16_t width, uint16_t height,
                               bool include_text)
{
    ...
    {
        // EAN-13 = 12 digits (7 modules each) + 3 guard patterns (3+5+3 modules) + quiet zones
        total_modules = 12 * DIGIT_WIDTH + GUARD_WIDTH_NORMAL * 2 + GUARD_WIDTH_CENTER;
    }
    else
    { // BARCODE_UPCA
        // UPC-A = 12 digits (7 modules each) + 3 guard patterns (3+5+3 modules) + quiet zones
        total_modules = 12 * DIGIT_WIDTH + GUARD_WIDTH_NORMAL * 2 + GUARD_WIDTH_CENTER;
    }
    ...
    x += draw_guard(bitmap, x, GUARD_PATTERN_NORMAL, GUARD_WIDTH_NORMAL,
                    module_width, bar_height, width);
    ...
    { // BARCODE_UPCA
        ...
        {
            uint8_t digit = CHAR_TO_DIGIT(digits[i]);
            x += draw_pattern(bitmap, PATTERN_L[digit], x, module_width, bar_height, width);
        }
    }

    // Draw center guard pattern
    x += draw_guard(bitmap, x, GUARD_PATTERN_CENTER, GUARD_WIDTH_CENTER,
                    module_width, bar_height, width);
    ...
    // Text rendering would go here if include_text is true
    // This would require a font implementation which is outside the scope
    // of this basic barcode generator
